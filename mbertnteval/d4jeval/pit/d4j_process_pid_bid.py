...
def d4j_pit_generate_mutants(d4j_project: D4jProject, pit_jar_path, output_dir, threads):
    try:
        ...
        raise e
    try:
        return pit_generate_mutants(d4j_project.jdk, cp,
...
def main_func(fix_commit_changes_csv, config):
    ...
    d4j_pit_generate_mutants(d4j_project, os.path.expanduser(config['pit_jar']), output_dir, config['exec']['threads'])


if __name__ == '__main__':
    args = get_args()
    main_func(args.fix_commit_changes_csv, args.config)
